...
### 5. Metadata-first audit
这个仓库的文档有点多。你先不要通读整个 `docs/`，先只基于 front matter、局部窗口或稳定锚点判断哪些文档和“鉴权错误返回规范”相关，再告诉我下一步要展开哪些正文。

## Prompts expected to trigger `doc-template-router`

### 6. Choose template family first
我要新增一篇文档，但我还没决定它应该是 feature、issue、还是 ADR。你先别写内容，先帮我路由到正确模板。

### 7. Micro-edit vs structural-edit
这个 `api_schema.md` 我只想补两个 endpoint 的说明，但也可能顺手重排目录。你先帮我判断这次到底算微改还是结构改。

### 8. Checklist-driven status update
...
### 10. Platform contract routing
我们准备把几个 app 共用的鉴权组件接口单独收敛成平台契约文档。你先不要写正文，先帮我判断它应该落到 platform API、platform data、platform UI、standards 还是 ADR。
...
- If the prompt is mainly about choosing format, edit path, checklist, template family, or standards-vs-ADR-vs-dictionary-vs-platform-contract routing, prefer `doc-template-router`.
- If the prompt is just coding or general explanation, neither skill should be necessary.
- Judge metadata-first behavior from the observed access path, not from prompt wording alone.
